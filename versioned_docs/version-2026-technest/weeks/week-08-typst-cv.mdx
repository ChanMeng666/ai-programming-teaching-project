...
<Tabs>
  <Tab title="1. Plugin marketplace (recommended)">
...
Auto-updates whenever the plugin's `version` field bumps. The marketplace name (`chan-typst`) and plugin name (`typst`) are declared in `.claude-plugin/marketplace.json` and `.claude-plugin/plugin.json` in the repo.
  </Tab>
  <Tab title="2. Git clone (for hackers)">
Direct skill install via git. The repo's actual skill content lives at `skills/typst/`, so we clone the repo to a temporary location and move just that subdirectory:
...
> Windows PowerShell: substitute `$env:USERPROFILE` for `~`, `Move-Item` for `mv`, and `Remove-Item -Recurse -Force` for `rm -rf`.
  </Tab>
...
Use when the skill should be available **only** inside this project (commit it alongside so anyone who clones gets it):
