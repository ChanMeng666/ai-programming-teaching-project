...
| 时间 | 活动 |
|---|---|
| 0 – 15 分钟 | **回顾与 Check-in。** 快速一轮：每个人念出自己第 2 周的网站 URL，并打开第 2 周末尾接通的那个轻量版 AI 克隆。挑出克隆给你的一句"不像你"的回答——今天就来修这个感觉。 |
...
<Note title="第 2 周已经有 key 了？">
如果你在第 2 周末尾已经创建了 Gemini API key，**直接复用** —— 跳到阶段 3（设计 persona）。如果你的第 2 周轻量版克隆本地能跑，那阶段 2 的 `.env.local` 和 Vercel env 接线也已经做完了。
</Note>
...
<Note title="预告第 4 周 —— RAG">
今天的克隆完全是 prompt-based ——`content/persona.md` 就是它的全部知识库。**第 4 周** 我们会接 Neon Postgres + `pgvector`，开始把真正的素材存进去，让 AI 从里面检索。这就是讲师在第 2 周末尾说的那句"*在企业里我们用向量数据库，今天先不上*"对应的 **RAG**（Retrieval-Augmented Generation）那一步。今天是过渡：一个能塞进 system prompt 的 persona。下周：一个能扩展到几千条素材的 persona。
</Note>
