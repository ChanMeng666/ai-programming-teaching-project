...
<Tabs>
  <Tab title="1. Git clone（推荐）">
直接把 skill 克隆进 Claude Code 的用户级 skills 目录。一条命令，macOS / Linux / Windows（Git Bash 或 WSL）通用：

```bash
git clone https://github.com/ChanMeng666/typst-claude-skill.git ~/.claude/skills/typst
```

> Windows PowerShell 用户：把 `~` 替换成 `$env:USERPROFILE`，目标路径就是 `$env:USERPROFILE\.claude\skills\typst`。
  </Tab>
...
这条路径在 plugin `version` 字段升级时会自动更新。marketplace 名字（`chan-typst`）和 plugin 名字（`typst`）声明在仓库的 `.claude-plugin/marketplace.json` 与 `.claude-plugin/plugin.json` 里。
  </Tab>
  <Tab title="3. 手动 ZIP">
机器上没有 Git 的同学：
...
2. 解压。
3. 把解压出来的整个文件夹移动并改名为 `~/.claude/skills/typst`（macOS / Linux）或 `%USERPROFILE%\.claude\skills\typst`（Windows）。
4. 确认 `SKILL.md` 在该文件夹的根。
  </Tab>
  <Tab title="4. 项目级（per-project）">
当 skill 应该**只**在这个项目内可用时使用（你可以把它和项目一起 commit，让 clone 的人都自动拿到）：
